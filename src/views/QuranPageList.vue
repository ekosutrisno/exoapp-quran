<template>
         ...
         <div class="max-w-7xl mx-auto p-4 relative">
            ...
            <div class="text-center">
               <div class="w-20 h-20  mx-auto border-2 rounded-full relative">
                  <div class="w-full h-full flex items-center justify-center rounded-full bg-yellow-200">
                        <h1 class="text-3xl font-semibold"> {{surah.number}} </h1>
                  </div>
               ...
               <h1 class="text-3xl md:text-5xl font-semibold my-4 text-gray-100 font-quran"> {{surah.name}} </h1>
               <h1 class="text-3xl md:text-5xl text-gray-100 font-quran"> {{surah.englishName}} </h1>
               <p class="font-semibold text-yellow-300 md:text-lg text-center"> {{surah.englishNameTranslation}}</p>
            </div>
            <div class="w-full md:mx-auto max-w-3xl -mb-24 bg-white z-40 shadow-lg mt-4 rounded-lg h-16">
               <div class="flex flex-col md:flex-row items-center justify-center text-xl md:text-3xl divide-y md:divide-y-0 md:divide-x h-full">
                  <div class="px-4 w-full text-center font-semibold text-green-600"> {{surah.revelationType}} </div>
                  <div class="px-4 w-full text-center font-semibold text-cyan-600"> {{surah.numberOfAyahs}} Ayahs </div>
               </div>
            </div>
         </div>
   </section>
   <section class="min-w-min bg-gray-200 pt-20 min-h-full">
     <div  class="max-w-7xl mx-auto px-4 pb-4 flex flex-col items-end space-y-2">
        <div v-for="ayah in surah.ayahs" :key="ayah.number" class="w-full group text-gray-600 transition max-w-screen-md mx-auto font-quran font-semibold text-right py-2 px-4 text-xl rounded-l-md border-r-2 border-transparent cursor-pointer hover:border-gray-400">
         <p> 
            <span class="group-hover:text-gray-900">{{ayah.text}}</span>   
               <span class="text-xl text-yellow-700 font-mono"> -{{replaceAr(ayah.numberInSurah)}}</span> 
         </p> 
        </div>
     </div>
   </section>
...
export default {
   setup(){
      ...
      const getSurahDetail = async () =>{
         await store.dispatch('surah/setSurah', route.params.surah_number);
      }
